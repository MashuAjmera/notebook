...
define([
  ...
  var RecentList = function(selector, options) {
    //mru
    /**
     * Constructor
     *
     ...
     */
    notebooklist.NotebookList.call(
      this,
      selector,
      $.extend(
        {
    ...
    this.events.on(
      ...
      $.proxy(this._kernelspecs_loaded, this)
    );
  };

  RecentList.prototype = Object.create(notebooklist.NotebookList.prototype); //mru

  RecentList.prototype.add_duplicate_button = function() {
    //mru
    /**
     * do nothing
     */
  ...
  RecentList.prototype._kernelspecs_loaded = function(event, kernelspecs) {
    //mru
  ...
  RecentList.prototype.sessions_loaded = function(d) {
    //mru
    this.sessions = d;
    if (!this.kernelspecs) {
      ...
      this.add_link(
        {
          ...
          type: "notebook",
          kernel_display_name:
            info && info.spec ? info.spec.display_name : session.kernel.name
        },
        item
      );
    }
  ...
  RecentList.prototype.add_link = function(model, item) {
    //mru
    notebooklist.NotebookList.prototype.add_link.apply(this, [model, item]);

    var recent_indicator = item.find(".item_buttons").text("");

    var that = this;
    var kernel_name = $("<div/>")
      .addClass("kernel-name")
      .text(model.kernel_display_name)
      .appendTo(recent_indicator);
